...
**Read first:** [the route's own doc](../src/app/items/CLAUDE.md), then
[services](../src/services/CLAUDE.md) and [`public/data/CLAUDE.md`](../public/data/CLAUDE.md)
...
the root `CLAUDE.md`: *data pages are statically generated; keep them that way.*
...
`CLAUDE.md` — they belong here, with a date on them.
...
read the same curves on purpose — see [the route's doc](../src/app/items/CLAUDE.md) — and that
...
- `src/app/items/CLAUDE.md` says how the route gets its data now, updated in the same change
