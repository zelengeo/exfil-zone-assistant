...
describe('the vendor record', () => {
    ...
    it('gives every vendor an org, a merchant and a mark', () => {
        for (const vendor of VENDOR_LIST) {
            expect(vendor.org, vendor.key).toBeTruthy();
            expect(vendor.merchant, `${vendor.key} merchant`).not.toBeNull();
            expect(vendor.icon, `${vendor.key} icon`).not.toBeNull();
            expect(vendor.isTrader, `${vendor.key} isTrader`).toBe(true);
        }
    });
